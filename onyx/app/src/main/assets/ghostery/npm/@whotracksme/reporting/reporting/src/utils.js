...
function requireIntOrNull(value, name) {
...
function intersectMapKeys(map1, map2) {
 ...
 */
function fastHash(str, { seed = 0, truncate = false, output } = {}) {
 ...
 */
function equalityCanBeProven(x, y) {
  if (
    x === true ||
    x === false ||
    x === null ||
...
function roundUpToNextUTCMidnight(unixEpoch) {
...
export { chunk, clamp, equalityCanBeProven, fastHash, flattenObject, intersectMapKeys, isNil, lazyInitAsync, parseUntrustedJSON, randBetween, requireArrayOfStrings, requireBoolean, requireInt, requireIntOrNull, requireObject, requireParam, requireString, requireStringOrNull, roundUpToNextUTCMidnight, split0 };
