...
import { getCurrentDay, newUTCDate, dateString } from '../utils/time.js';
    ...
    if (
      ...
      const newTrackerMap = state.urlParts
        ...
        .params.reduce((hash, kv) => {
          ...
          const key = this.hashTokens ? cachedMD5(k) : k;
          if (this.qsWhitelist.isSafeKey(tracker, key)) {
            return hash;
          }
          const tok = this.hashTokens ? cachedMD5(v) : v;
          if (!hash[key]) {
            hash[key] = {};
          }
          hash[key][tok] = today;
          ...
          if (
            ...
            this.qsWhitelist.addSafeKey(
              tracker,
              this.hashTokens ? key : cachedMD5(key),
...
export { TokenExaminer as default };
