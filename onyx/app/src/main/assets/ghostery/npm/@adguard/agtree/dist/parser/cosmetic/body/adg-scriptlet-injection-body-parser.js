 ...
 */
class AdgScriptletInjectionBodyParser extends BaseParser {
    /**
    ...
    static parse(raw, options = defaultParserOptions, baseOffset = 0) {
        ...
        while (offset < closingParenthesesIndex) {
            ...
            if (parameterList.children.length > 0) {
                if (raw[offset] !== COMMA) {
                    throw new AdblockSyntaxError(sprintfExports.sprintf(AdgScriptletInjectionBodyParser.ERROR_MESSAGES.EXPECTED_COMMA, raw[offset]), baseOffset + offset, baseOffset + raw.length);
                }
                // Eat the comma
                offset += 1;
                // Skip whitespace
            ...
            if (raw[offset] === SINGLE_QUOTE || raw[offset] === DOUBLE_QUOTE) {
                ...
                throw new AdblockSyntaxError(sprintfExports.sprintf(AdgScriptletInjectionBodyParser.ERROR_MESSAGES.EXPECTED_QUOTE, raw[offset]), baseOffset + offset, baseOffset + raw.length);
            }
        }
