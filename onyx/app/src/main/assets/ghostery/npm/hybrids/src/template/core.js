...
import { isLocalizeEnabled, get } from '../localize.js';
import { insertRule, inject } from './layout.js';
import { getPlaceholder, getMeta, removeTemplate, getTemplateEnd } from './utils.js';
      ...
      if (equal) {
        ...
        if (
          ...
          if (!localizedKey.match(PLACEHOLDER_REGEXP_ONLY)) {
            ...
            const resultKey = get(localizedKey, context).replace(
              /\${(\d+)}/g,
      ...
      if (node.nodeType === globalThis.Node.ELEMENT_NODE) {
        ...
        for (const attr of Array.from(node.attributes)) {
          ...
          if (useLayout && name.startsWith("layout") && value) {
            ...
            const className = insertRule(node, name.substr(6), value);
