...
import Options, { isGloballyPaused, getPausedDetails, MODE_ZAP, MODE_DEFAULT, revokeGlobalPause, findParentDomain } from '../../../store/options.js';
import ElementPickerSelectors from '../../../store/element-picker-selectors.js';
import TabStats from '../../../store/tab-stats.js';
import ManagedConfig from '../../../store/managed-config.js';
import Resources from '../../../store/resources.js';
import Notification from '../store/notification.js';
import sleep from '../assets/sleep.js';
import { showAlert, clearAlert } from '../components/alert.js';
import __vite_glob_0_19 from './clear-cookies.js';
import __vite_glob_0_21 from './menu.js';
import __vite_glob_0_27 from './tracker-details.js';
import __vite_glob_0_23 from './protection-status.js';
import __vite_glob_0_24 from './report-category.js';
import __vite_glob_0_26 from './report-form.js';
import __vite_glob_0_25 from './report-confirm.js';
import __vite_glob_0_28 from './whotracksme.js';
import __vite_glob_0_22 from './pause-assistant.js';
...
async function togglePause(host, event) {
  ...
  if (paused) {
    ...
    await store.set(options, {
      paused: {
        [stats.hostname]: {
          revokeAt: pauseType && Date.now() + 60 * 60 * 1e3 * pauseType
        }
      }
    });
  }
  showAlert(html`
    <panel-alert type="danger">
      ${paused ? msg`Ghostery has been resumed on this site.` : msg`Ghostery is paused on this site.`}
...
const Main = {
  ...
  paused: ({ options, stats }) => store.ready(options, stats) && getPausedDetails(options, stats.hostname),
  globalPause: ({ options }) => store.ready(options) && isGloballyPaused(options),
  contentBlocksSelectors: ({ elementPickerSelectors, stats }) => store.ready(stats, elementPickerSelectors) && elementPickerSelectors.hostnames[stats.hostname]?.length || 0,
  consentManaged: ({ resources, options, stats, paused }) => store.ready(resources, options, stats) && !paused && options.blockAnnoyances && resources.autoconsent[stats.domain],
  render: ({
    options,
    stats,
    notification,
    ...
    <template layout="column grow relative">
      ${store.ready(options, managedConfig) && html`
        ${options.terms && html`
          <ui-header>
            ${store.ready(stats) && (managedConfig.disableUserControl || options.mode === MODE_ZAP && paused) && html`<ui-text type="label-m">${stats.displayHostname}</ui-text>`}
            ${options.mode === MODE_DEFAULT && html`<ui-icon name="logo" slot="icon" layout="size:2.5"></ui-icon>`}
            ${options.mode === MODE_ZAP && html`<ui-icon
              name="logo-zap"
              slot="icon"
              layout="margin:left:-1"
            ></ui-icon>`}
            ${!managedConfig.disableUserControl && html`
              <ui-action slot="actions">
                <a href="${router.url(__vite_glob_0_21)}" data-qa="button:menu">
                  <ui-icon name="menu" color="primary"></ui-icon>
                </a>
              </ui-action>
            `}
          </ui-header>
          ${store.ready(stats) && !managedConfig.disableUserControl && (options.mode !== MODE_ZAP || !paused) && html`
            <panel-actions hostname="${stats.displayHostname}">
              ...
              <panel-actions-button>
                <a href="${router.url(__vite_glob_0_24)}">
    ...
    "/pages/settings/index.html#@settings-website-details?domain=" + stats.hostname
        ...
        `}
        ${options.terms && store.ready(stats) && !managedConfig.disableUserControl && (options.mode === MODE_DEFAULT || globalPause) && html`
          <panel-pause
          ...
          >
            ${!!paused?.revokeAt && !paused.assist && html`
        ...
        `}
        ${options.terms && store.ready(stats) && !managedConfig.disableUserControl && options.mode === MODE_ZAP && !globalPause && html`
          <panel-zap
                ...
                >
                  <ui-tooltip position="bottom" slot="actions">
                    ...
                    <ui-action-button layout="size:4.5">
                      <a href="${router.url(__vite_glob_0_28)}">
                        <ui-icon name="whotracksme" color="primary"></ui-icon>
                      </a>
                    </ui-action-button>
                  </ui-tooltip>
                  ${!stats.groupedTrackers.length && html`
      ...
      (tracker) => html`
        ...
        trackerId: tracker.id
                                  ...
                                  >
                                    ...
                                    ${tracker.blocked && html`<ui-icon
                                      name="block-s"
                                      color="danger-primary"
                                      data-qa="icon:tracker:${tracker.id}:blocked"
                                    ></ui-icon>`}
                                    ${tracker.modified && html`<ui-icon
                                      name="eye"
                                      color="brand-primary"
                                ...
                                ${options.terms && !managedConfig.disableUserControl && !paused && !globalPause && html`
    ...
    last: managedConfig.disableUserControl || !store.ready(notification)
        ...
        ${!managedConfig.disableUserControl && store.ready(notification) && !store.error(notification) && html`<panel-notification></panel-notification>`}
      `}
    </template>
  `
};
