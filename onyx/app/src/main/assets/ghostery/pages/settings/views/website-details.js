...
import __vite_glob_0_24 from './tracker-details.js';
import __vite_glob_0_26 from './website-clear-cookies.js';
import store from '../../../npm/hybrids/src/store.js';
import router from '../../../npm/hybrids/src/router.js';
import { html } from '../../../npm/hybrids/src/template/index.js';

function removeDomain(tracker) {
  return ({ options, domain }) => toggleDomain(options, tracker.id, domain);
}
...
const __vite_glob_0_27 = {
  [router.connect]: {
    stack: () => [__vite_glob_0_24, __vite_glob_0_26],
    replace: true
  },
  domain: "",
  ...
  trackers: ({ options, domain }) => store.ready(options) ? Object.entries(options.exceptions).filter(([, { domains }]) => domains.includes(domain)).map(([id]) => id).sort((a, b) => a.localeCompare(b)).map((id) => store.get(Tracker, id)).map(
    (tracker) => store.error(tracker) ? { id: tracker.id, name: tracker.id } : tracker
  ) : [],
  elementPickerSelectors: store(ElementPickerSelectors),
  selectors: ({ elementPickerSelectors, domain }) => store.ready(elementPickerSelectors) && elementPickerSelectors.hostnames[domain]?.join("\n") || "",
    ...
    <template layout="contents">
      <settings-page-layout layout="gap:4">
        <div layout="column items:start gap">
          ...
          ${paused.revokeAt !== void 0 && !paused.assist && html`
    ...
    pointerEvents: "none"
        ...
        >
          ...
          <settings-table>
            ...
            `}
            ${trackers.map(
    (tracker) => !store.pending(tracker) && html`
                  <div
                    layout="grid:2 gap:2"
                    layout@768px="grid:2fr|2fr|3fr gap:4"
                  >
                    ${store.ready(tracker) && html`<ui-action>
                      <a
      ...
      tracker: tracker.id
                    ...
                    ${!store.ready(tracker) && html`<ui-text type="label-m" mobile-type="label-s">
                      ${tracker.name}
                    </ui-text>`}
                    <ui-text
                      type="label-m"
        ...
        ${html`
          <div layout="row gap:5">
            ...
            <ui-button disabled="${clearedCookies}">
              <a
                href="${router.url(__vite_glob_0_26, { domain })}"
        ...
        `}
        ${hasWTMStats(topLevelDomain) && html`
